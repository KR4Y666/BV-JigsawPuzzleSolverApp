...
picture = Image.open('puzzle4.png')
...
pieces_sorted = sorted([[cnt.shape[0], i] for i, cnt in enumerate(contours)], reverse=True)[:7]
true_pieces = [contours[s[1]] for s in pieces_sorted] 
...
def matching(input1, input2):
  ...
  for i in range(0, center1.shape[0], STEP_1):
    ...
    for j in range(0, center2.shape[0], STEP_2):
      ...
      if type2 != type2:
        if ((abs(h1-h2) < PRECISION) & (abs(w1-w2) < PRECISION)) or ((abs(h1-2) < PRECISION) & (abs(w1-h2) < PRECISION)):
