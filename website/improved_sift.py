...
def crop_image(image):
    ...
    ret, fg = cv2.threshold(distance_transform, 0.02*distance_transform.max(), 255, 0)
...
def sift_algorithm(object, template, color):
    ...
    # using SIFT to detect and compute keypoints (kp) and descriptors(des)
    object_kp, object_des = sift.detectAndCompute(object,None) 
    templ_kp, templ_des = sift.detectAndCompute(template,None)

    # using FLANN for fast nearest neighbors search 
    ...
    # there have to be at least min_matches to find the object
    min_matches = 10

    #print("Matches: " + str(len(best_matches)))
    # get matches
    if len(best_matches) > min_matches:
        ...
        #print("Only " + str(len(best_matches)) + "/" + str(min_matches) + " found.")
        matches_mask = None
...
#puzzle_area_img_path = './puzzle_data/puzzle4_area.jpg'
...
#puzzle_template_img = cv2.imread(puzzle_template_img_path)
#puzzle_area_img = cv2.imread(puzzle_area_img_path)
...
def search_position(piece, template, number_of_pieces):
    ...
    x_start, y_start, x_target, y_target, result_img = improved_sift(piece, template) 
...
#_,_,_,_, result = improved_sift(puzzle_piece_img, puzzle_template_img)
#cv2.imshow('image1',result)
#cv2.waitKey(0)
#cv2.destroyAllWindows()

#result2 = search_position(puzzle_piece_img, puzzle_template_img, 100)
#cv2.imshow('image2', result2)
#cv2.waitKey(0)
#cv2.destroyAllWindows()

# search for the puzzle piece that fits in to a particular part of the puzzle template
def search_piece(template_area, pieces):
    ...
    # piece_of_template = template[left_corner:width, left_corner:height]

    return improved_sift(template_area, pieces)[4]

#cv2.imshow('image3',search_piece(puzzle_area_img, pieces_img))
#cv2.waitKey(0)
#cv2.destroyAllWindows()
